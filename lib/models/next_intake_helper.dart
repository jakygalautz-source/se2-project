...
class NextIntakeHelper {
  static NextIntakeModel? calculate({
    required DateTime now,
    required ReminderTimeModel reminderTimeModel,
    required List<Medication> medications,
  }) {
    final List<_CandidateIntake> candidates = [];

    for (final dayPart in DayPart.values) {
      final matchingMedications = medications.where((medication) {
        return medication.intakes.any(
          (intake) => intake.dayPart == dayPart.name && intake.amount > 0,
        );
      }).toList();

      if (matchingMedications.isEmpty) continue;

      final matchingReminderMedications = matchingMedications.where((
        medication,
      ) {
        return medication.intakes.any(
          (intake) =>
      ...
      candidates.add(
        _CandidateIntake(
          ...
          medications: matchingMedications,
          medicationsWithReminder: matchingReminderMedications,
        ),
      );
    }
    ...
    final minutesUntil = next.scheduledDateTime.difference(now).inMinutes;
